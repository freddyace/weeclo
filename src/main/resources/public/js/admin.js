 ...
 * */
 ...
 function userExists(userEmail, password){
   ...
   httpRequest.open('GET', "http://"+
     ...
     userEmail.value+"&password="+password.value, true);
   httpRequest.setRequestHeader('Content-Type','application/json');
   httpRequest.send();
   console.log("Endpoint Hit: "+"http://"+
     serverAddress+":5000/login/credentials/?email="+
...
/* Validates the data from the modal */
 function validateModalForm(firstnameValue,lastnameValue,emailValue,addressLine1Value,zipcodeValue,desiredUsernameValue, intendedPasswordValue, intendedPasswordConfirmationValue, idValue){
   var missedFieldsArray = [];
   if(firstnameValue.length==0||lastnameValue.length==0||emailValue.length==0||addressLine1Value.length==0||
     zipcodeValue.length==0||desiredUsernameValue.length==0||intendedPasswordValue.length==0||intendedPasswordConfirmationValue.length==0||idValue.length==0){
     //redo form by re-routing user back to step 2
     if(firstnameValue.length==0){missedFieldsArray.push(" First Name");}
     if(lastnameValue.length==0){missedFieldsArray.push(" Last Name");}
     if(emailValue.length==0){missedFieldsArray.push(" Email");}
     if(addressLine1Value.length==0){missedFieldsArray.push(" Address");}
     if(zipcodeValue.length==0){missedFieldsArray.push(" Zip");}
     if(desiredUsernameValue.length==0){missedFieldsArray.push(" Desired Username");}
     if(intendedPasswordValue.length==0){missedFieldsArray.push(" Password");}
     if(intendedPasswordConfirmationValue.length==null){missedFieldsArray.push(" Password Confirmation");}
     if(idValue.length==0){missedFieldsArray.push(" Unique ID")}
