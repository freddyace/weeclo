...
import com.weeclo.demo.sessionMangement.SessionManager;
...
public class Controller implements Promisable{
    ...
    public ResponseEntity<String> login(HttpServletRequest httpServletRequest,
        ...
        System.out.println("Token passed through header is: " + httpServletRequest.getHeader("token"));
        String token = httpServletRequest.getHeader("token");
        //If no token is passed, execute the following
        if (token == null){
            ...
            try {
                ...
                if (list.size() == 1) {
                    ...
                    if (passwordEncoder.matches(password, userEntity.getPassword())) {
                        ...
                        if (sessionManager.sessionExistsSearchByUserEntity(userEntity)) {
                    ...
                    ///*******************************//
                    /*error/issue occured*/
                    else return new ResponseEntity<>("Incorrect credentials", headers, HttpStatus.FORBIDDEN);

                }
                if (list.size() > 1) {
                    return new ResponseEntity<>("Too many matches....how many accounts do you have???", HttpStatus.NOT_ACCEPTABLE);
                }
            } catch (PersistenceException p) {
                log.error(p.getMessage());
                p.printStackTrace();
            }
    }else if(token!=null && email=="" &&password==""){
            if(sessionManager.sessionExistsSearchByToken(token)){
               return new ResponseEntity<>( sessionManager.retrieveSessionAsStringByToken(token), HttpStatus.OK);
            }
        }
        return new ResponseEntity<>(new JSONObject(new WeeCloSessionNotFoundResponse()).toString(), HttpStatus.SERVICE_UNAVAILABLE);
    }
